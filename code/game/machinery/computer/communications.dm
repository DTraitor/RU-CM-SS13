...
/obj/structure/machinery/computer/communications/Topic(href, href_list)
	...
	switch(href_list["operation"])
		...
		if("setmsg1")
			stat_msg1 = reject_bad_text(trim(copytext_char(sanitize(input("Первая строка", "Введите текст сообщения", stat_msg1) as text|null), 1, 40)), 40)
			updateDialog()

		if("setmsg2")
			stat_msg2 = reject_bad_text(trim(copytext_char(sanitize(input("Вторая строка", "Введите текст сообщения", stat_msg2) as text|null), 1, 40)), 40)
			updateDialog()
